...
const Pay = ({ cart }) => {
  ...
  const { isPending, isError, mutate, data } = useMutation({
    mutationFn: async (cart) => {
      ...
      const response = await axios.post("http://localhost:8000/microservice-payment", {
        cart,
      });
  ...
  return (
    <div className="bg-blue-100 flex flex-col items-center justify-center gap-4 py-8 rounded-xl">
      <div className="flex flex-col gap-12">
        ...
        <div className="flex items-center gap-2 text-gray-500">
          ...
          <span className="font-semibold text-xs">**** 6789</span>
